# -*- coding: utf-8 -*-
"""
Created on Mon Jan 17 16:02:39 2022
...
class AlphaNet(nn.Module):
    ...
    def ts_pool(self, data, stride, method):
        ...
        numpy_fill(pool_data)
        return torch.tensor(pool_data)
...
def validset_loss(model, valid_days): # 给出模型在验证集上的整体损失
    ...
    for date in valid_days:
        data = pd.read_pickle('/Users/liupeilin/Desktop/AlphaNet/data/' + date + '.pkl')
        feature = list(data['X'])
        label = list(data['Y2'])
        valid_data = []
...
def model_backtest(alphanet, all_stock_pool, train_days, test_days): # 给出模型在训练集和测试集上的IC和RankIC，同时记录测试集上的alpha表
    ...
    alpha.to_csv('/Users/liupeilin/Desktop/alpha_in.csv', index=False)
...
def model_train(train_days, valid_days, batch_size=1000, max_epoch=10, lr=1e-5, patience=2):
    ...
    alphanet = AlphaNet(1, 324, 128, 30, 1, 9)
    alphanet = torch.load('/Users/liupeilin/Desktop/AlphaNet/MSE_model/Y2/model_1000-1100.pkl')
    loss_func = nn.MSELoss()
    # loss_func = myLoss(mode='IC')
    optimizer = RMSprop(alphanet.parameters(), lr=lr)
    
    path = '/Users/liupeilin/Desktop/AlphaNet/MSE_model/Y2/'
    name = 'model_1000-1100'
...
def main():
    ...
    tradingdays = tradingdays[(tradingdays['Date'] >= '2015-02-25') & (tradingdays['Date'] < '2018-10-31')]
    tradingdays = list(tradingdays['Date'])
    
    train_days = tradingdays[0:801]
    valid_days = tradingdays[901:1001]
    test_days = tradingdays[1001:1100]
    
    
    all_stock_pool = pd.read_csv('/Users/liupeilin/Desktop/股票数据/all_stock_pool.csv')
    
    # model = torch.load('/Users/liupeilin/Desktop/AlphaNet/IC_model/Y2/model_49.pkl')
    time1 = time()
    # 模型训练
    model, train_loss_list, valid_loss_list = model_train(train_days, valid_days, max_epoch=150, lr=1e-5, patience=2)
    time2 = time()
